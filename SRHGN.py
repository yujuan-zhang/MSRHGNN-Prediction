    ...
    def forward(self, G, h):
        ...
        with G.local_scope():
            ...
            G.multi_update_all(
                {
                    etype: (
                        fn.u_mul_e(f'v_{e_id}', f'a_{e_id}', 'm'),
                        fn.sum('m', 'z')
                    )
...
class SRHGN(nn.Module):
    ...
    def forward(self, G, target):
        ...
        for ntype in G.ntypes:
            ...
            h[ntype] = self.pre_transform[n_id](G.nodes[ntype].data['x'].to('cuda:0'))
        ...
        out_dict = {}
        for ntype in G.ntypes:
            out_dict[ntype] = self.out(h[ntype])

        return out_dict, h[target], attns