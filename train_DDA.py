...
import time
from global_pc_grad_magnitude_positive import *
from model import MRSHGNN
from metric import *
import warnings
...
if __name__ == '__main__':
    ...
    parser.add_argument('--neighbor', type=int, default=9, help='neighbor')
    ...
    parser.add_argument('--threshold_usvt', default='0.3', type=float, help='threshold of usvt')
    ...
    parser.add_argument('--hgt_out_dim', default=64, type=int, help='heterogeneous graph transformer output dimension')
    ...
    for i in range(args.k_fold):
        ...
        model = MRSHGNN(args, g_m_drug, g_m_disease, g_s_drug, g_s_disease)
        ...
        optimizer = GlobalPCGradMagnitudePositive(optimizer, 2, b=0, r=0.5, e=1e-5, device=device)
        ...
        for epoch in range(args.epochs):
            ...
            l_con, train_output, _, _ = model(g_m_drug, g_m_disease, g_s_drug, g_s_disease, pos_r, pos_d,
                                        drug_feature, disease_feature, protein_feature, X_train_p, X_train_n, neg_candidates_train_r, neg_candidates_train_d, drdr_graph, didi_graph)

            l_train = cross_entropy(train_output, torch.flatten(Y_train))

            losses = [l_train, l_con]
            optimizer.zero_grad()
            ...
            with torch.no_grad():
                ...
                _, test_score, _, _ = model(g_m_drug, g_m_disease, g_s_drug, g_s_disease,
                                      pos_r, pos_d, drug_feature, disease_feature, protein_feature, X_test_p, X_test_n, neg_candidates_test_r, neg_candidates_test_d, drdr_graph, didi_graph)
            ...
            time1 = end - start
            show = [epoch + 1, round(time1, 2), l_con, l_train, round(AUC, 5), round(AUPR, 5), round(accuracy, 5),
                       round(precision, 5), round(recall, 5), round(f1, 5), round(mcc, 5)]
            print('\t\t'.join(map(str, show)))

            save_dir = r'./Results/' + args.dataset + '/' + time_stamp + '/'
            ...
            if AUC > best_auc:
                ...
                np.save(os.path.join(save_dir, f'label{i}.npy'), Y_test)
                np.save(os.path.join(save_dir, f'score{i}.npy'), test_prob)
