...
def get_data(args):
    ...
    dre = pd.read_csv(args.data_dir + 'drug_ie_sim.csv').iloc[:, 1:].to_numpy()

    dip = pd.read_csv(args.data_dir + 'DiseasePS.csv').iloc[:, 1:].to_numpy()
    dig = pd.read_csv(args.data_dir + 'DiseaseGIP.csv').iloc[:, 1:].to_numpy()
    die = pd.read_csv(args.data_dir + 'disease_ie_sim.csv').iloc[:, 1:].to_numpy()
    ...
    drug_GAE = pd.read_csv(args.data_dir + 'drug_f512_feature.csv', header=None).to_numpy()
    disease_GAE = pd.read_csv(args.data_dir + 'disease_f512_feature.csv', header=None).to_numpy()
    protein_GAE = pd.read_csv(args.data_dir + 'protein_f512_feature.csv', header=None).to_numpy()

    data['dr_fusion'] = drug_GAE
    data['di_fusion'] = disease_GAE

    data['drug_gae'] = (drf + drg + dre + drug_GAE) / 4
    data['disease_gae'] = (dip + dig + die + disease_GAE) / 4
    data['protein_gae'] = (prs + prg_r + prg_d) / 3
...
def negative_sample_kmeans(data, one_index, zero_index, args, n_clusters=10, per_cluster_ratio=0.1):
    ...
    for (d, di) in zero_index:
        feat = np.concatenate([drug_feat[d], dis_feat[di]])
    ...
    cluster_labels = kmeans.fit_predict(zero_features)

    selected_neg = []
    for c in range(n_clusters):
        cluster_idx = np.where(cluster_labels == c)[0]
        cluster_samples = [zero_index[i] for i in cluster_idx]
        k = int(len(cluster_samples) * per_cluster_ratio)
        if k > 0:
            selected_neg.extend(random.sample(cluster_samples, k))

    num_neg = int(args.negative_rate * len(one_index))
    if len(selected_neg) > num_neg:
        selected_neg = random.sample(selected_neg, num_neg)

    return np.array(selected_neg)
...
def data_processing(data, args):
    ...
    neg_samples = negative_sample_kmeans(data, one_index, zero_index, args, n_clusters=20, per_cluster_ratio=0.05)
    ...
    drs_mean = (data['drf'] + data['drg'] + data['dre']) / 3
    dis_mean = (data['dip'] + data['dig'] + data['die']) / 3
...
def dgl_similarity_graph(data, args):
    ...
    if args.dataset == 'B-dataset':
        drdr_graph.ndata['sim_feature'] = torch.tensor(data['drug_gae'])
        didi_graph.ndata['sim_feature'] = torch.tensor(data['disease_gae'])
    else:
        drdr_graph.ndata['sim_feature'] = torch.tensor(data['drug_gae'])
        didi_graph.ndata['sim_feature'] = torch.tensor(data['disease_gae'])
